...
function validateLangfuseCredentials() {
  if (!process.env.LANGFUSE_PUBLIC_KEY ||
    ...
    throw new Error('Langfuse credentials are not configured. Please add LANGFUSE_PUBLIC_KEY and LANGFUSE_SECRET_KEY in Vercel Dashboard → Settings → Environment Variables, then redeploy. Get credentials from https://cloud.langfuse.com → Settings → API Keys');
  }
}
...
export async function createPrompt(data: LangfusePromptData): Promise<any> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 403) {
      throw new Error(`Invalid Langfuse credentials. Please check your LANGFUSE_PUBLIC_KEY and LANGFUSE_SECRET_KEY in Vercel Dashboard → Settings → Environment Variables, then redeploy. Get credentials from https://cloud.langfuse.com → Settings → API Keys. Error: ${error}`);
